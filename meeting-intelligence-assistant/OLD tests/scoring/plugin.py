...
class ScoringPlugin:
    ...
    def pytest_runtest_makereport(self, item: Any, call: Any) -> Any:
        ...
        if report.passed:
            ...
            # Skipped OLD tests earn 0 but don't count as failed
            earned = 0
            passed = False
            error_msg = "SKIPPED"
        else:
    ...
    def pytest_terminal_summary(self, terminalreporter: Any, exitstatus: int, config: Any) -> None:
        ...
        for file_path, tests in sorted(self.session.by_file().items()):
            ...
            tr.write_line(f"  {file_path}: {file_earned:.1f}/{file_max:.1f} pts ({passed}/{total} OLD tests)")

            # Show individual failing OLD tests
            for test in tests:
                if not test.passed:
